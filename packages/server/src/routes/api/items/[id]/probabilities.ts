...
/**
 * GET /api/items/:id/probabilities - Get item category probabilities
 */
export const GET: RouteHandler = async (req) => {
  try {
    ...
    if (item.auctionId) {
      auction = await db.query.auctions.findFirst({
    ...
    return Response.json({
      ...
      auction: auction ? { ...auction, scraper: auctionScraper } : null,
    });
  } catch (error) {
    console.error("Error fetching item probabilities:", error);
    return Response.json({ error: "Failed to fetch item category probabilities" }, { status: 500 });
  }
};
